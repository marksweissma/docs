...
description: Migrate markdown and text files into the Moxn Knowledge Base
---

Import existing documentation from your filesystem into the Moxn Knowledge Base. The `@moxn/kb-migrate` CLI reads markdown and text files, splits them into sections, uploads images, and creates documents via the API.

<Tip>
If you have `@moxn/kb-cli` installed, you can run `knowledge-base import-local` instead of `npx @moxn/kb-migrate local`.
</Tip>
...
Import under a specific path prefix in the Knowledge Base:
