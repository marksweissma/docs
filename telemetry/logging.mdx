...
```python
# Method 1: In create_llm_event_from_parsed_response
event = session.create_llm_event_from_parsed_response(
    parsed_response=parsed,
    attributes={
...
# Method 2: On the span (affects all events in span)
async with client.span(session, metadata={"customer_id": "123"}) as span:
    span.set_attribute("query_type", "password_reset")
    ...
```
...
Log errors alongside successful calls:
...
async with client.span(session) as span:
    try:
        ...
        span.set_attribute("error", "rate_limit")
        span.set_attribute("error_message", str(e))
        # Event not logged, but span captures the error
        raise
```
    ...
    async with MoxnClient() as client:
        ...
        async with client.span(
            ...
            try:
                ...
                validation_errors = [str(err) for err in e.errors()]
                span.set_attribute("output_valid", False)
                result = None
