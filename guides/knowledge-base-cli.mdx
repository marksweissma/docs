...
description: Install the knowledge-base CLI to give AI coding agents direct access to Moxn Documents
---

The **knowledge-base CLI** is the fastest setup for AI coding agents in the terminal. It uses API key authentication and works as a [Claude Code skill](https://docs.anthropic.com/en/docs/claude-code/skills) — no MCP configuration required. For web and desktop apps, see [MCP Quick Start](/quickstart-documents).
...
This installs the `knowledge-base` command globally.
...
```bash
knowledge-base find --type document --limit 1
```
...
This creates `.claude/skills/knowledge-base/SKILL.md` in your current directory.

**Global** (available in all projects):

```bash
knowledge-base install-skill --global
```

This creates the skill in `~/.claude/skills/knowledge-base/SKILL.md`.
...
> "Search my knowledge base for onboarding docs"

> "Create a new document at /engineering/api-reference with an overview section"

> "Read the auth guide from Moxn"

The agent will call `knowledge-base` commands and get JSON responses — no MCP server needed.
