...
from moxn_types.telemetry import SpanContext
...
async def analyze_sentiment(
    ...
    async with client.span(
        ...
        span.set_attribute("sentiment", result)
        return result
...
async def extract_entities(
    ...
    async with client.span(
        ...
        result = json.loads(parsed.candidates[0].content[0].text)

        span.set_attribute("entity_count", sum(len(v) for v in result.values()))
        return result
...
async def conditional_workflow(document: str):
    async with MoxnClient() as client:
        async with client.span(..., name="conditional_analysis") as root_span:
            # First: classify the document
            doc_type = await classify_document(client, document, root_span.context)
