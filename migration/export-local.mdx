...
description: Download Moxn Knowledge Base documents as markdown files
---

Export your Knowledge Base documents to local markdown files. The tool fetches documents via the API, converts sections to markdown, and downloads any attached images, PDFs, and CSVs.

<Tip>
If you have `@moxn/kb-cli` installed, you can run `knowledge-base export-local` instead of `npx @moxn/kb-migrate export`.
</Tip>
...
The exported files mirror your Knowledge Base hierarchy:
...
<AccordionGroup>
  ...
  <Accordion title="Export is slow">
    Large knowledge bases with many images or attachments take longer due to file downloads. Use `--base-path` to export a smaller subset, or `--modified-after` to export only recent changes.
  </Accordion>
</AccordionGroup>
