...
description: Migrate pages and databases from Notion into the Moxn Knowledge Base
---

Import your Notion workspace into the Moxn Knowledge Base using the `@moxn/kb-migrate` CLI. It extracts pages, converts blocks into sections, downloads images, and recreates Notion databases as KB databases with tag-backed columns.

<Tip>
If you have `@moxn/kb-cli` installed, you can run `knowledge-base import-notion` instead of `npx @moxn/kb-migrate notion`.
</Tip>
...
See [Permission Model](/guides/knowledge-base#permission-model) for details on how permissions cascade.
