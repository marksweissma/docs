...
Documents and sections can link to each other — wiki-style bidirectional links. Link inline in the prose with a `<moxn:ref>` tag, or attach a link without touching the body using the [`documents` tool](/reference/mcp/documents) with `action: "add_reference"`. See [Backlinks](/concepts/backlinks).
