...
description: Author and manage agent skills, commands, and agents in Moxn — git-versioned, published to GitHub, with a fast local dev loop
---

Moxn is a **source of truth for your agent skills** (and commands and agents). You author them in Moxn — with the same branches, versioning, and collaboration you get for documents — then publish them to GitHub as an installable marketplace that works across Claude Code, Codex, and Cursor. A local **dev loop** lets you iterate against your Moxn content in seconds, without publishing.

<Note>
**Looking for skills management?** You're in the right place. A **skill** (`SKILL.md`) is the core unit here; Moxn organizes skills into **plugins** and distributes them through a **marketplace**. If you just want to write and share skills, follow [Create & author](/guides/marketplaces/authoring) — a plugin can be as small as a single skill.
</Note>
...
flowchart LR
  ...
  A -.->|"context plugin dev / refresh<br/>(live working state)"| L["Local dir<br/>claude --plugin-dir"]
...
- **The dev plane (dotted)** is how *you* iterate. `context plugin dev` / `plugin refresh` pull a plugin's **live branch content straight from Moxn** into a local directory — **no GitHub, no publish involved** — and `claude --plugin-dir` loads it. This is the fast loop, covered in [The dev loop](/guides/marketplaces/dev-loop).
...
| Your local dev copy | **Your machine** (`./.moxn/dev/<plugin>`) | Materialized by `plugin dev`; never published |
...
<Steps>
  ...
  <Step title="Develop">
    Iterate with the local dev loop — edit in Moxn, `plugin refresh`, `/reload-plugins`, test in Claude Code. See [The dev loop](/guides/marketplaces/dev-loop).
  </Step>
  <Step title="Publish">
  ...
  <Step title="Distribute (optional)">
    Reach Claude Cowork users directly — a personal self-serve install, or an admin-driven org-wide rollout. See [Distribute to your team](/guides/marketplaces/distribution).
  </Step>
</Steps>
